...
//! bit-by-bit (see [`XorGateAux`]), every bit is checked with the same Mul
//! machinery (including self-product checks `bx·bx` and `by·by` that bind the
//! prover to consistent values), and the XOR output share is rebuilt as a
//! weighted sum of `bx + by − 2·(bx·by)` per bit.  Linear gates (Add, AddConst,
...
/// Index range reserved for a single XOR gate's bit-level zero-shares.
///
/// Each XOR gate verifies 32 bits × 3 product checks:
///   - `bx·by` (the AND share)
///   - `bx·bx` (boolean check for x-bits: b²=b for bits)
///   - `by·by` (boolean check for y-bits: b²=b for bits)
/// so it consumes 96 consecutive zero-share indices, offset past the
/// Mul/Xor gate indices so the two indexing spaces never collide.
const XOR_ZERO_SHARES_PER_GATE: usize = 96;
...
/// left input, the 32 bits of the right input, and three sets of ZKBoo
/// product shares per bit:
///   - `bit_prod`: `bx_i * by_i`  (the AND product)
///   - `bit_xx`:   `bx_i * bx_i`  (boolean check: b²=b for bits)
///   - `bit_yy`:   `by_i * by_i`  (boolean check: b²=b for bits)
/// These are freshly generated by the prover (not seed-derivable), so they
/// MUST be included in the commitment bytes for the XOR verification to bind.
...
pub struct XorGateAux {
    ...
    pub bit_prod: [u32; 32],
    /// This party's ZKBoo product share of `bit_x_i * bit_x_i` for bit `i`.
    /// Used to enforce the boolean property: b² = b for bits.
    pub bit_xx: [u32; 32],
    /// This party's ZKBoo product share of `bit_y_i * bit_y_i` for bit `i`.
    /// Used to enforce the boolean property: b² = b for bits.
    pub bit_yy: [u32; 32],
}
...
impl XorAuxData {
    fn new() -> Self {
        Self {
            ...
            bit_xx: Vec::with_capacity(32),
            bit_yy: Vec::with_capacity(32),
        }
    }
}
        ...
        match gate {
            ...
            Gate::Xor {
                    ...
                    [(&mut bit_x_shares, *left), (&mut bit_y_shares, *right)]
                {
                    ...
                    bit_shares[0].shares[2] =
                        bit_shares[0].shares[2].wrapping_sub(err0.wrapping_add(err1));
                }
                ...
                for i in 0..32 {
                    ...
                    let mut bxx = vec![0u32; num_parties];
                    let mut byy = vec![0u32; num_parties];
                    for p in 0..num_parties {
                        let next = (p + 1) % num_parties;
                        // Zero-share for bx·by product
                        let r_prod =
                            zero_share(&party_seeds[p].0, &party_seeds[next].0, base + i * 3);
                        ...
                        // Zero-share for bx·bx product (boolean check)
                        let r_xx =
                            zero_share(&party_seeds[p].0, &party_seeds[next].0, base + i * 3 + 1);
                        bxx[p] = mul_output_share(
                            bx.shares[p],
                            bx.shares[p],
                            bx.shares[next],
                            bx.shares[next],
                        ...
                        // Zero-share for by·by product (boolean check)
                        let r_yy =
                            zero_share(&party_seeds[p].0, &party_seeds[next].0, base + i * 3 + 2);
                        byy[p] = mul_output_share(
                            by.shares[p],
                            by.shares[p],
                            by.shares[next],
                            by.shares[next],
        ...
        .map(|p| {
            ...
            let xor_bit_shares: Vec<XorGateAux> = xor_aux_data
                ...
                .map(|aux| XorGateAux {
                    ...
                    bit_xx: std::array::from_fn(|i| aux.bit_xx[i][p]),
                    bit_yy: std::array::from_fn(|i| aux.bit_yy[i][p]),
                })
                .collect();
            PartyView {
                party_idx: p,
            ...
            Gate::Xor {
                ...
                if let Some(next) = next_opened {
                    ...
                    for i in 0..32 {
                        ...
                        let bxx = aux.bit_xx[i];
                        let byy = aux.bit_yy[i];

                        // 1. The AND product share must be a valid ZKBoo
                        //    Mul-share of (bx, by).
                        let r_prod = zero_share(party_seed, next.seed, base + i * 3);
                        if bp != mul_output_share(bx, by, bx_next, by_next, r_prod) {
                            return Err(crate::MpcithError::ConsistencyCheckFailed(party_idx));
                        }

                        // 2. Self-product share for x-bits: bxx must be a valid
                        //    ZKBoo Mul-share of (bx, bx).  This commits the
                        //    prover to the self-product, binding bx to a value
                        //    whose square is consistent.
                        let r_xx = zero_share(party_seed, next.seed, base + i * 3 + 1);
                        if bxx != mul_output_share(bx, bx, bx_next, bx_next, r_xx) {
                            return Err(crate::MpcithError::ConsistencyCheckFailed(party_idx));
                        }

                        // 3. Self-product share for y-bits: byy must be a valid
                        //    ZKBoo Mul-share of (by, by).
                        let r_yy = zero_share(party_seed, next.seed, base + i * 3 + 2);
                        if byy != mul_output_share(by, by, by_next, by_next, r_yy) {
                            return Err(crate::MpcithError::ConsistencyCheckFailed(party_idx));
                        }

                        // 4. Linear reconstruction of the x/y wire shares.
                        let weight = 1u32 << i;
                        x_recon = x_recon.wrapping_add(bx.wrapping_mul(weight));
                        y_recon = y_recon.wrapping_add(by.wrapping_mul(weight));
                        // 5. Linear reconstruction of the XOR output share.
...
mod tests {
    ...
    #[test]
    fn test_xor_self_product_tampering_detected() {
        // Tampering with the self-product shares (bit_xx or bit_yy) must be
        // rejected: the ZKBoo product check of (bx, bx) or (by, by) will fail.
        let circuit = make_xor_circuit();
        let mut rng = thread_rng();
        let seeds: Vec<PartySeed> = (0..3).map(|_| PartySeed::random(&mut rng)).collect();
        let exec = run_mpc_emulation(&circuit, &[0x0Fu32, 0x5A, 0x26], &seeds, &mut rng).unwrap();
        ...
        // Tamper party 0's bit_xx[0] for the first Xor gate.
        let mut tampered_aux = exec.views[0].xor_bit_shares.clone();
        tampered_aux[0].bit_xx[0] ^= 1;
        ...
        assert!(
            ...
            .is_err(),
            "tampered bit_xx must be rejected by ZKBoo product check"
        );

        // Tamper party 0's bit_yy[0] for the first Xor gate.
        let mut tampered_aux2 = exec.views[0].xor_bit_shares.clone();
        tampered_aux2[0].bit_yy[0] ^= 1;
        ...
        assert!(
            ...
            .is_err(),
            "tampered bit_yy must be rejected by ZKBoo product check"
        );
    }
}
