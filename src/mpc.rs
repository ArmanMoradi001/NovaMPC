...
//! AssertEq) are computed locally from input shares.
//!
...
//! shares (freshly re-shared from a global RNG) are stored in the proof.
...
/// Broadcast messages are no longer needed — the verifier recomputes all
/// input shares from the seed via [`recompute_linear_shares`], so Mul/Xor
/// gate input consistency is guaranteed by the seed derivation.
...
mod tests {
    ...
    fn test_view_consistency() {
        ...
        for view in &exec.views {
            ...
            verify_party_view(&circuit, &ws, view.party_idx).unwrap();
        }
    }
}
